    ...
    public class TileDownloader
    {
        ...
        private void TaskProc(object o)
        {
            ...
            while (!mStop)
            {
                ...
                {   
                    //文件已经存在,或下载成功
                    if (Util.isFileExists(task.file) || download(task.url, task.file))
                    {
                        ++mDownloadedTile;

                        updateProgress(mDownloadedTile, mTotalTile, DOWNLOAD_STATE_DOWNLOADING);
                        string[] fileNamePart = task.file.Split('\\');
                        string fileName = "...";
                        for (int fileNamePartIndex = fileNamePart.Length - 4; fileNamePartIndex <=fileNamePart.Length - 1; fileNamePartIndex++) {
                             fileName += "\\"+fileNamePart[fileNamePartIndex];
                        }
                       
                        String strLog = "[完成/总数:" +
                            mDownloadedTile + "/" + mTotalTile + "，级:" +
                            task.z + "/" + mZoomArray[mZoomArray.Count - 1] + "，横:" +
                            task.x + "/" + mRightTopTile.x + "，纵:" +
